...
class BrainDataset(torch_data.Dataset):
    def __init__(self,  mode, transforms, base_dir, domains: list, tile_z_dim = 3, extern_norm_fn = None, location_scale=False):
        ...
        #     self.pid_curr_load = self.scan_ids
        # self.pid_curr_load = self.sample_list[domains[0]]

        self.normalize_op = extern_norm_fn([itm['img_fid'] for _, itm in self.sample_list[self.domains[0]].items()])

        print(f'For {self.phase} on {[_dm for _dm in self.domains]}')
    ...
    def __search_samples(self):#, scan_ids):
        ...
        out_list = {}
        out_list[self.domains[0]] = {}
        
        if self.domains[0] == "t1":
            folder = "images"
        elif self.domains[0] == "dwi":
            folder = "dwi"
        elif self.domains[0] == "flair":
            folder = "flair"
        else:
        ...
        for file in sorted(os.listdir(base_subdir_images)):
            ...
            _img_fid = os.path.join(base_subdir_images, file)
            _lb_fid = os.path.join(base_subdir_labels, file)
            
            curr_dict["img_fid"] = _img_fid
            curr_dict["lbs_fid"] = _lb_fid
            out_list[self.domains[0]][_img_fid] = curr_dict
